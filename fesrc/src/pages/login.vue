    ...
    sendEmail() {
      ...
      if (this.sendEmailStatus === 0 || this.sendEmailStatus === 2) {
        ...
        if (this.url != "") {
          sync
            .POST("/api/sendMail", {
              html: `<h3>【ccsu-bbs】这里是长沙学院论坛，您正在注册账号，验证码是<VerificationCode>，转发可能导致邮箱绑定错误，请不要告诉任何人你的验证码！（此消息五分钟内有效）</h3>`,
              title: "长沙学院论坛验证码",
              emailTo: this.signUp.email,
              type: 1
            })
            .then(data => {
              console.log(data);
        ...
        this.sendEmailInterval = setInterval(() => {
          const hadTime =
            60 - (new Date().getTime() - this.sendEmailTime) / 1000;
